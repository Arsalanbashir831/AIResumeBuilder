...
export default function TemplateEditor() {
	...
	const handleDownloadClick = async () => {
		...
		const updatedSnapshotUrl = await sendTemplateDataToIframe();
		if (updatedSnapshotUrl) {
			const pdf = new jsPDF({
				orientation: "portrait",
				unit: "px",
				format: [900, 1200],
			});
			pdf.addImage(updatedSnapshotUrl as string, "JPEG", 0, 0, 900, 1200);
			pdf.save("resume.pdf");
		} else {
			alert("Snapshot is not available for download");
		}
	...
	const handleSubmit = async (authToken: string) => {
		try {
			...
			if (snapshotUrl) {
				...
				const response = await fetch(updatedSnapshotUrl as string);
		...
		<div className='flex flex-col h-screen p-8 pt-24 container mx-auto'>
			<header className='flex justify-between items-center mb-4'>
				<Button
					...
					onClick={() => {
						router.push("/dashboard/");
						localStorage.removeItem("template_ss");
					}}>
					<ArrowLeft size={24} />
					To Dashboard
				</Button>
			...
			<div className='flex flex-col lg:flex-row-reverse gap-5'>
				{showModal && (
					<SubscriptionModal onClose={handleClose} onPayNow={handlePayNow} />
				)}
					...
					<div className='flex justify-between items-center mb-4'>
						...
						{isEditResume && (
							<>
								<Button
									variant='default'
									onClick={credits > 0 ? handleDownloadClick : handlePayNow}
